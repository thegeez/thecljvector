(ns net.thegeez.thecljvector.layout
...
(defmacro application [name args & transformations]
  ...
  `(enlive/deftemplate ~name ~application-template
     ...
     [:#flash-info] (when-let [info# (or (get-in (first ~args) [:request :flash :info])
                      ...
                      (enlive/content info#))
     [:a#menu-home] (enlive/set-attr :href (get-in (first ~args) [:menu-links :home]))
     [:a#menu-add-link] (enlive/set-attr :href (get-in (first ~args) [:menu-links :add-link]))
     ~@transformations))
